...
const RequestMaterial: React.FC = () => {
  const { courseId, resourceId, yearId, subjectId } = useParams();
  ...
  const subject = selectedYear?.subjects.find(s => s.id === subjectId);
  ...
  const validate = (): FormErrors => {
    ...
    if (!hasConfirmedScreenshot) {
      newErrors.confirmation = 'Please check the box to confirm you will attach the proof';
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (hasErrors) {
      const fieldOrder: (keyof FormErrors)[] = ['name', 'email', 'phone', 'transactionId', 'confirmation'];
    ...
    const emailBody = `
VIDYATE ACADEMIC HUB - MATERIAL REQUEST FORM
...
DECLARATION
-----------
I hereby request the above material and confirm that I will attach the payment screenshot to this email. I understand that the material will be shared after manual verification.
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center animate-scale-up">
        ...
        <div className="glass p-10 md:p-16 rounded-3xl border-[#10B981] border shadow-2xl">
          ...
          <h1 className="text-2xl md:text-3xl font-black mb-3 tracking-tight" style={{ color: 'var(--text-main)' }}>Final Step!</h1>
          <p className="opacity-70 text-sm md:text-base font-medium mb-6 leading-relaxed" style={{ color: 'var(--text-main)' }}>
            Your email app should have opened. <span className="text-[#405cff] font-bold">Attach your screenshot</span> and click <b>Send</b>.
          </p>
          <div className="bg-[#10B981]/10 border border-[#10B981]/20 p-4 rounded-xl mb-10">
            <p className="text-[11px] font-black uppercase tracking-widest text-[#10B981]">
              Next Step Verification
            </p>
            <p className="text-xs mt-1 font-medium opacity-80" style={{ color: 'var(--text-main)' }}>
              Once you send the email, our team will verify your payment and <b>respond within the next few hours</b> with your materials.
            </p>
          </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-8 md:py-12 animate-subtle-fade">
      ...
      <div className="mb-8 flex justify-between items-center">
        <button 
          onClick={() => navigate(-1)} 
          className="text-theme-muted hover:text-[#405cff] flex items-center gap-2 font-black text-[11px] uppercase tracking-widest transition-colors"
        >
      ...
      <div className="flex flex-col lg:flex-row gap-8 md:gap-12 items-start">
          ...
          <div className="glass p-8 rounded-3xl border text-center shadow-lg" style={{ borderColor: 'var(--glass-border)' }}>
            ...
            <div className="md:hidden w-full mb-8">
               ...
               <div className="grid grid-cols-1 gap-2">
                 <a href="tez://upi/pay?pa=godowner1976@okicici&pn=Vidyate&am=49&cu=INR" className="flex items-center justify-between p-3 px-4 rounded-xl bg-white/5 border hover:bg-white/10 transition-all group" style={{ borderColor: 'var(--glass-border)' }}>
                 ...
                 <a href="phonepe://pay?pa=godowner1976@okicici&pn=Vidyate&am=49&cu=INR" className="flex items-center justify-between p-3 px-4 rounded-xl bg-white/5 border hover:bg-white/10 transition-all group" style={{ borderColor: 'var(--glass-border)' }}>
                   <div className="flex items-center gap-3">
                 ...
                 <a href="paytmmp://pay?pa=godowner1976@okicici&pn=Vidyate&am=49&cu=INR" className="flex items-center justify-between p-3 px-4 rounded-xl bg-white/5 border hover:bg-white/10 transition-all group" style={{ borderColor: 'var(--glass-border)' }}>
                   <div className="flex items-center gap-3">
            ...
            <div className="p-4 rounded-xl bg-white/5 border border-dashed text-left" style={{ borderColor: 'var(--glass-border)' }}>
              <div className="flex justify-between items-center font-black text-sm" style={{ color: 'var(--text-main)' }}>
                ...
                <div className="flex items-center text-[#405cff]">
                  ...
                  <span className="text-lg">49.00</span>
                </div>
              </div>
              <p className="text-xs opacity-40 mt-2 leading-relaxed">Processing is handled manually. Expected delivery within 24 hours via Email.</p>
            </div>
            ...
            <div className="mt-6 p-4 bg-yellow-500/5 border border-yellow-500/20 rounded-xl text-left">
               <div className="flex gap-2 items-center mb-1">
                 ...
                 <p className="text-[11px] font-black uppercase tracking-widest text-yellow-600">Attachment Required</p>
               </div>
               <p className="text-xs opacity-70" style={{ color: 'var(--text-main)' }}>
                 You will need to manually attach the payment screenshot in the email app after submitting the form below.
               </p>
            </div>
          </div>
        </div>
        ...
        <div className="w-full lg:w-7/12 order-2 lg:order-1 glass p-6 md:p-10 rounded-3xl border shadow-xl" style={{ borderColor: 'var(--glass-border)' }}>
          <div className="mb-8">
            <h2 className="text-2xl font-black tracking-tight mb-2" style={{ color: 'var(--text-main)' }}>Request Form</h2>
            <div className="flex items-center flex-wrap gap-2">
              <span className="bg-[#405cff]/10 text-[#405cff] text-[10px] font-black uppercase px-2 py-0.5 rounded border border-[#405cff]/20">
                {subject.name}
              </span>
              <span className="opacity-40 text-[10px] uppercase font-black tracking-widest" style={{ color: 'var(--text-main)' }}>
                {selectedYear?.name} • {resource?.name}
              </span>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="pt-4 border-t" style={{ borderColor: 'var(--glass-border)' }}>
              <div className="flex items-start gap-3 cursor-pointer group">
                ...
                />
                <label htmlFor="confirm-screenshot" className="text-sm font-medium opacity-70 group-hover:opacity-100 transition-opacity cursor-pointer" style={{ color: 'var(--text-main)' }}>
                  I confirm that I have made the payment and I will attach the screenshot in the email client.*
                </label>
              </div>
              ...
              <div className="hidden md:block bg-red-500/5 border border-red-500/20 rounded-lg p-4">
                <div className="flex gap-3">
                  ...
                  <div className="space-y-1">
                    <p className="text-[11px] font-black uppercase tracking-widest text-red-600">Device Compatibility Note</p>
                    <p className="text-[11px] font-medium leading-relaxed text-red-600/80">
                      If you've already paid but your device or browser prevents you from opening the email app or attaching files, <b>please refill this form using your smartphone</b> to complete the request and easily attach your payment screenshot.
                    </p>
                  </div>
                </div>
              </div>
